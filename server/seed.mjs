...
async function seed() {
  ...
  try {
    ...
    for (const challenge of writingChallengesData) {
      await connection.execute(
        `INSERT INTO writingChallenges (topic, title, prompt, proficiencyLevel, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, NOW(), NOW())`,
        [challenge.topic, challenge.title, challenge.prompt, challenge.proficiencyLevel]
      );
    }
